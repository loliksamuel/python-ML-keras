...
def back_test(filename, symbol, skipRows, names_input, names_output,start_date, end_date):
    ...
    for i in range(lenxx-1):#0 to 13894   #for index, row in df_oc.iterrows():
        ...
        currBarDirection  = df_y_observed[(i+0):(i+1)]
        ...
        currBarDir = currBarDirection.values[0]
        #print(' next Bar is Up? ', nextBarIsUp)
        print(' curr Bar direction? ', currBarDirection.values[0])
        ...
        if  y_pred_curr == 1 :# green bar prediction
            ...
            if  currBarDir == +1 :
                ...
                print('doing nothing due to low volatility : +0.1 >',percentCurr)
